...
def get_website_status(url):
    ...
    Returns:
        A tuple (status_code, is_reachable) where:
            ...
            - is_reachable: True if the website is reachable (including 403 and 406), False otherwise.
    """
    try:
        response = requests.get(url, timeout=10)
        # Treat 403 and 406 as reachable
        if response.status_code in [403, 406, 404]:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        future_to_url = {executor.submit(get_website_status, row['Website']): row for _, row in data.iterrows() if pd.notna(row['Website'])}
