...
def read_file(filepath):
    """
    Reads a file and updates the preparing file list.
    """
    if filepath.endswith('.csv'):
        ...
        return
    preparing_files.append({"path": filepath, "data_type": tk.StringVar(value="other")})
    update_preparing_list()

def update_preparing_list():
    """
    Updates the displayed list of preparing files.
    """
    for widget in preparing_list_frame.winfo_children():
        widget.destroy()

    for i, file_info in enumerate(preparing_files):
        file_path = file_info["path"]
        file_type_var = file_info["data_type"]

        file_label = ttk.Label(preparing_list_frame, text=file_path.split('/')[-1])
        file_label.grid(row=i, column=0, padx=5, pady=5)

        type_menu = ttk.OptionMenu(preparing_list_frame, file_type_var, file_type_var.get(), *FILE_TYPES)
        type_menu.grid(row=i, column=1, padx=5, pady=5)

        delete_button = ttk.Button(preparing_list_frame,
                                    text="Delete",
                                    command=lambda idx=i: delete_preparing_file(idx))
        delete_button.grid(row=i, column=2, padx=5, pady=5)

def delete_preparing_file(index):
    """
    Deletes a file from the preparing file list.
    """
    del preparing_files[index]
    update_preparing_list()
...
def upload_all_files():
    ...
    """
    if not preparing_files:
        messagebox.showerror("Error", "No files to upload.")
        return
    for file_info in preparing_files:
...
# Section for files preparing to load
preparing_label = ttk.Label(root, text="Files Preparing to Load:", anchor="w")
preparing_label.pack(anchor="w", padx=20)

preparing_list_frame = ttk.Frame(root)
preparing_list_frame.pack(pady=10, padx=20, fill=tk.BOTH, expand=True)
